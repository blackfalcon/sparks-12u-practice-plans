// DO NOT EDIT! 
// This doc was auto generated from ./src/scripts/build-page-imports.js 
// Sat Nov 12 2022 20:03:19 GMT-0800 (Pacific Standard Time)
      
// Import primary markdown tools
import {InternalMarkdownWrapper} from '../components/rawMarkdownWrapper';
      
// internal markdown docs
import Game091622Page from './static/blog/game091622.md'; 
import Practice002Page from './static/blog/practice002.md'; 
import Practice003Page from './static/blog/practice003.md'; 
import Practice004Page from './static/blog/practice004.md'; 
import Practice005Page from './static/blog/practice005.md'; 
import Practice006Page from './static/blog/practice006.md'; 
import Practice007Page from './static/blog/practice007.md'; 
import Practice008Page from './static/blog/practice008.md'; 
import Practice009Page from './static/blog/practice009.md'; 
import Practice010Page from './static/blog/practice010.md'; 
import Practice011Page from './static/blog/practice011.md'; 
import Practice012Page from './static/blog/practice012.md'; 
import WarmupsPage from './static/blog/warmups.md'; 
import AboutPage from './static/general/about.md'; 
import CatchingdrillsPage from './static/general/catchingdrills.md'; 
import FieldingdrillsPage from './static/general/fieldingdrills.md'; 
import GamewarmupsPage from './static/general/gamewarmups.md'; 
import ProperhittingPage from './static/general/properhitting.md'; 
import ProperthrowingPage from './static/general/properthrowing.md'; 
import ThrowingprogressionPage from './static/general/throwingprogression.md'; 
import ThrowingwarmupsPage from './static/general/throwingwarmups.md'; 
import HomePage from './static/home.md'; 

      
// internal markdown docs
export class Game091622 extends InternalMarkdownWrapper {
  readme = Game091622Page
}

export class Practice002 extends InternalMarkdownWrapper {
  readme = Practice002Page
}

export class Practice003 extends InternalMarkdownWrapper {
  readme = Practice003Page
}

export class Practice004 extends InternalMarkdownWrapper {
  readme = Practice004Page
}

export class Practice005 extends InternalMarkdownWrapper {
  readme = Practice005Page
}

export class Practice006 extends InternalMarkdownWrapper {
  readme = Practice006Page
}

export class Practice007 extends InternalMarkdownWrapper {
  readme = Practice007Page
}

export class Practice008 extends InternalMarkdownWrapper {
  readme = Practice008Page
}

export class Practice009 extends InternalMarkdownWrapper {
  readme = Practice009Page
}

export class Practice010 extends InternalMarkdownWrapper {
  readme = Practice010Page
}

export class Practice011 extends InternalMarkdownWrapper {
  readme = Practice011Page
}

export class Practice012 extends InternalMarkdownWrapper {
  readme = Practice012Page
}

export class Warmups extends InternalMarkdownWrapper {
  readme = WarmupsPage
}

export class About extends InternalMarkdownWrapper {
  readme = AboutPage
}

export class Catchingdrills extends InternalMarkdownWrapper {
  readme = CatchingdrillsPage
}

export class Fieldingdrills extends InternalMarkdownWrapper {
  readme = FieldingdrillsPage
}

export class Gamewarmups extends InternalMarkdownWrapper {
  readme = GamewarmupsPage
}

export class Properhitting extends InternalMarkdownWrapper {
  readme = ProperhittingPage
}

export class Properthrowing extends InternalMarkdownWrapper {
  readme = ProperthrowingPage
}

export class Throwingprogression extends InternalMarkdownWrapper {
  readme = ThrowingprogressionPage
}

export class Throwingwarmups extends InternalMarkdownWrapper {
  readme = ThrowingwarmupsPage
}

export class Home extends InternalMarkdownWrapper {
  readme = HomePage
}


    